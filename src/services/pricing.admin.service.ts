...
export const tierService = {
  ...
  get: async (id: string) => {
    const { data } = await axios.get(`/pricing/priority-tiers/${id}/`);
    return data;
  },
  create: async (payload: Partial<DevicePriorityTier>) => {
    const { data } = await axios.post('/pricing/priority-tiers/', payload);
  ...
  importCsv: async (file: File): Promise<ImportResult> => {
    ...
    const { data } = await axios.post('/pricing/priority-tiers/import_csv/', formData, {
      headers: { 'Content-Type': 'multipart/form-data' },
    });
    return data;
  },
  exportCsv: async () => {
    const { data } = await axios.get('/pricing/priority-tiers/export_csv/', { responseType: 'blob' });
...
export const adminEstimateService = {
  ...
  exportCsv: async () => {
    const { data } = await axios.get('/pricing/estimates/export_csv/', { responseType: 'blob' });
    return new Blob([data], { type: 'text/csv' });
  },
};